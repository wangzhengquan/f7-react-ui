...
uNode.createElement = function (html) {
    if (/[<>]/.test(html)) {
        return (0, _htmlparser2.default)(html).children[0];
    } else {
        return new uNode({
            type: 'element',
            children: [],
...
uNode.createText = function (data, noTrans) {
    return new uNode({
        type: 'text',
        'data': noTrans ? data : _utils2.default.unhtml(data || '')
    });
};
    ...
    switch (node.type) {
        case 'root':
            for (var i = 0, ci; ci = node.children[i++];) {
                //插入新行
                if (formatter && ci.type == 'element' && !_dtd2.default.$inlineWithA[ci.tagName] && i > 1) {
...
function isText(node, arr) {
    if (node.parentNode.tagName == 'pre') {
        ...
        arr.push(notTransTagName[node.parentNode.tagName] ? _utils2.default.html(node.data) : node.data.replace(/[ ]{2}/g, ' &nbsp;'));
    }
}
...
function isElement(node, arr, formatter, current) {
    ...
    if (node.attrs) {
        ...
        for (var a in attrs) {
            ...
            //有可能做的不够
            attrhtml.push(a + (attrs[a] !== undefined ? '="' + (notTransAttrs[a] ? _utils2.default.html(attrs[a]).replace(/["]/g, function () {
                return '&quot;';
            }) : _utils2.default.unhtml(attrs[a])) + '"' : ''));
        }
        attrhtml = attrhtml.join(' ');
    }
    arr.push('<' + node.tagName + (attrhtml ? ' ' + attrhtml : '') + (_dtd2.default.$empty[node.tagName] ? '\/' : '') + '>');
    //插入新行
    if (formatter && !_dtd2.default.$inlineWithA[node.tagName] && node.tagName != 'pre') {
    ...
    if (!_dtd2.default.$empty[node.tagName]) {
        if (formatter && !_dtd2.default.$inlineWithA[node.tagName] && node.tagName != 'pre') {
...
uNode.prototype = {
    ...
    innerHTML: function innerHTML(htmlstr) {
        if (this.type != 'element' || _dtd2.default.$empty[this.tagName]) {
            return this;
        }
        if (_utils2.default.isString(htmlstr)) {
            if (this.children) {
            ...
            this.children = [];
            var tmpRoot = (0, _htmlparser2.default)(htmlstr);
    ...
    appendChild: function appendChild(node) {
        if (this.type == 'root' || this.type == 'element' && !_dtd2.default.$empty[this.tagName]) {
            if (!this.children) {
                this.children = [];
            }
            if (node.parentNode) {
    ...
    setAttr: function setAttr(attrName, attrVal) {
        ...
        if (_utils2.default.isObject(attrName)) {
    ...
    getNodesByTagName: function getNodesByTagName(tagNames) {
        tagNames = _utils2.default.trim(tagNames).replace(/[ ]{2,}/g, ' ').split(' ');
        var arr = [],
            me = this;
        _utils2.default.each(tagNames, function (tagName) {
    ...
    setStyle: function setStyle(name, val) {
        function exec(name, val) {
            ...
            if (val) {
                cssStyle = name + ':' + _utils2.default.unhtml(val) + ';' + cssStyle;
            }
        }
        ...
        this.setAttr('style', _utils2.default.trim(cssStyle));
    },
    hasClass: function hasClass(className) {
        if (this.hasAttr('class')) {
            var classNames = this.getAttr('class').split(/\s+/),
                hasClass = false;
            _dom2.default.each(classNames, function (key, item) {
                if (item === className) {
                    hasClass = true;
                }
            });
    ...
    removeClass: function removeClass(className) {
        if (this.hasAttr('class')) {
            ...
            this.setAttr('class', _utils2.default.trim(cl).replace(/[ ]{2,}/g, ' '));
        }
    },
    /**
