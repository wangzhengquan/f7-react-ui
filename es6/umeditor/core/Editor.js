...
var _EventBase = require('./EventBase');
...
function setValue(form, editor) {
    var textarea;
    if (editor.textarea) {
        if (_utils2.default.isString(editor.textarea)) {
            for (var i = 0, ti, tis = _domUtils2.default.getElementsByTagName(form, 'textarea'); ti = tis[i++];) {
    ...
    if (!textarea) {
        form.appendChild(textarea = _domUtils2.default.createElement(document, 'textarea', {
...
function loadPlugins(me) {
    //初始化插件
    for (var pi in _editor2.default.plugins) {
        if (me.options.excludePlugins.indexOf(pi) == -1) {
            _editor2.default.plugins[pi].call(me);
            me.plugins[pi] = 1;
        }
    }
    me.langIsReady = true;
 ...
 * - ***selection*** 编辑区域的选区对象
 */
var Editor = _editor2.default.Editor = function (options) {
    var me = this;
    me.uid = uid++;
    _EventBase2.default.call(me);
    me.commands = {};
    me.options = _utils2.default.extend(_utils2.default.clone(options || {}), UMEDITOR_CONFIG, true);
    me.shortcutkeys = {};
    me.inputRules = [];
    me.outputRules = [];
    //设置默认的常用属性
...
Editor.prototype = {
    ...
    setOpt: function setOpt(key, val) {
        ...
        _utils2.default.extend(this.options, obj, true);
    },
    ...
    destroy: function destroy() {
        ...
        if (container.contains(textarea)) {
            (0, _dom2.default)(textarea).insertBefore(container);
        }
        container.innerHTML = '';

        _domUtils2.default.remove(container);
        _editor2.default.clearCache(me.id);
        //trace:2004
        for (var p in me) {
    ...
    initialCont: function initialCont(holder) {
        ...
        if (holder) {
            ...
            if (holder && /script|textarea/ig.test(holder.tagName)) {
                ...
                this.options.initialContent = (0, _htmlparser2.default)(holder.value || holder.innerHTML || this.options.initialContent).toHtml();
    ...
    render: function render(container) {
        var me = this,
            ...
            return parseInt((0, _dom2.default)(container).css(attr));
        };

        if (_utils2.default.isString(container)) {
            container = document.getElementById(container);
        }
        if (container) {
            this.id = container.getAttribute('id');
            _editor2.default.setEditor(this);
            _utils2.default.cssRule('edui-style-body', me.options.initialStyle, document);
            ...
            if (options.initialFrameWidth) {
                ...
                //都没给值，先写死了
                options.minFrameWidth = options.initialFrameWidth = (0, _dom2.default)(container).width() || _editor2.default.defaultWidth;
            }
            if (options.initialFrameHeight) {
                options.minFrameHeight = options.initialFrameHeight;
            } else {

                options.initialFrameHeight = options.minFrameHeight = (0, _dom2.default)(container).height() || _editor2.default.defaultHeight;
            }
            ...
            if (this.options.autoHeightEnabled) {
                ...
                if (_browser2.default.ie && _browser2.default.version <= 6) {
                    container.style.height = height;
                    container.style.setExpression('height', 'this.scrollHeight <= ' + height + ' ? "' + height + 'px" : "auto"');
                }
            } else {
                (0, _dom2.default)(container).height(height);
            }
    ...
    _setup: function _setup(cont) {
        ...
        me.body = cont;
        me.$body = (0, _dom2.default)(cont);
        me.selection = new _Selection2.default(document, me.body);
        me._isEnabled = false;
        //gecko初始化就能得到range,无法判断isFocus了
        var geckoSel;
        if (_browser2.default.gecko && (geckoSel = this.selection.getNative())) {
            geckoSel.removeAllRanges();
        }
        this._initEvents();
        //为form提交提供一个隐藏的textarea
        for (var form = cont.parentNode; form && form.nodeType !== 9 && !_domUtils2.default.isBody(form); form = form.parentNode) {
            if (form.tagName == 'FORM') {
                me.form = form;
                if (me.options.autoSyncData) {
                    (0, _dom2.default)(cont).on('blur', function () {
                        setValue(form, me);
                    });
                } else {
                    (0, _dom2.default)(form).on('submit', function () {
                        setValue(this, me);
                    });
                }
                break;
        ...
        if (_domUtils2.default.isEmptyNode(me.body)) {
        ...
        if (!_browser2.default.ie || _browser2.default.ie9above) {
            ...
            (0, _dom2.default)(me.body).on('blur focus', function (e) {
    ...
    sync: function sync(formId) {
        var me = this,
            form = formId ? document.getElementById(formId) : _domUtils2.default.findParent(me.body.parentNode, function (node) {
    ...
    setHeight: function setHeight(height, notSetHeight) {
        ...
        if (this.options.autoHeightEnabled) {
            (0, _dom2.default)(this.body).css({
                'min-height': height + 'px'
            });
            if (_browser2.default.ie && _browser2.default.version <= 6 && this.container) {
                this.container.style.height = height;
                this.container.style.setExpression('height', 'this.scrollHeight <= ' + height + ' ? "' + height + 'px" : "auto"');
            }
        } else {
            (0, _dom2.default)(this.body).height(height);
        }
    ...
    setWidth: function setWidth(width) {
        ...
        (0, _dom2.default)(this.body).width(width - (0, _dom2.default)(this.body).css('padding-left').replace('px', '') * 1 - (0, _dom2.default)(this.body).css('padding-right').replace('px', '') * 1);
        this.fireEvent('resize');
    },
    addshortcutkey: function addshortcutkey(cmd, keys) {
        ...
        _utils2.default.extend(this.shortcutkeys, obj);
    },
    ...
    getContent: function getContent(cmd, fn, notSetCursor, ignoreBlank, formatter) {
        var me = this;
        if (cmd && _utils2.default.isFunction(cmd)) {
            fn = cmd;
            cmd = '';
        }
        ...
        var root = _editor2.default.htmlparser(me.body.innerHTML, ignoreBlank);
    ...
    getAllHtml: function getAllHtml() {
        ...
        if (_browser2.default.ie && _browser2.default.version > 8) {
            var headHtmlForIE9 = '';
            _utils2.default.each(me.document.styleSheets, function (si) {
                headHtmlForIE9 += si.href ? '<link rel="stylesheet" type="text/css" href="' + si.href + '" />' : '<style>' + si.cssText + '</style>';
            });
            _utils2.default.each(me.document.getElementsByTagName('script'), function (si) {
                headHtmlForIE9 += si.outerHTML;
            });
        }
        return '<html><head>' + (me.options.charset ? '<meta http-equiv="Content-Type" content="text/html; charset=' + me.options.charset + '"/>' : '') + (headHtmlForIE9 || me.document.getElementsByTagName('head')[0].innerHTML) + headHtml.join('\n') + '</head>' + '<body ' + (_browser2.default.ie && _browser2.default.version < 9 ? 'class="view"' : '') + '>' + me.getContent(null, null, true) + '</body></html>';
    },
    /**
    ...
    getPlainTxt: function getPlainTxt() {
        var reg = new RegExp(_domUtils2.default.fillChar, 'g'),
            html = this.body.innerHTML.replace(/[\n\r]/g, ''); //ie要先去了\n在处理
        html = html.replace(/<(p|div)[^>]*>(<br\/?>|&nbsp;)<\/\1>/gi, '\n').replace(/<br\/?>/gi, '\n').replace(/<[^>/]+>/g, '').replace(/(\n)?<\/([^>]+)>/g, function (a, b, c) {
            return _dtd2.default.$block[c] ? '\n' : b ? b : '';
        });
    ...
    getContentTxt: function getContentTxt() {
        var reg = new RegExp(_domUtils2.default.fillChar, 'g');
        //取出来的空格会有c2a0会变成乱码，处理这种情况\u00a0
        return this.body[_browser2.default.ie ? 'innerText' : 'textContent'].replace(reg, '').replace(/\u00a0/g, ' ');
    },
    ...
    setContent: function setContent(html, isAppendTo, notFireSelectionchange) {
        ...
        var root = _editor2.default.htmlparser(html);
        ...
        if (me.options.enterTag == 'p') {
            ...
            var child = this.body.firstChild,
                tmpNode;
            if (!child || child.nodeType == 1 && (_dtd2.default.$cdata[child.tagName] || isCdataDiv(child) || _domUtils2.default.isCustomeNode(child)) && child === this.body.lastChild) {
                this.body.innerHTML = '<p>' + (_browser2.default.ie ? '&nbsp;' : '<br/>') + '</p>' + this.body.innerHTML;
            } else {
                var p = me.document.createElement('p');
                while (child) {
                    while (child && (child.nodeType == 3 || child.nodeType == 1 && _dtd2.default.p[child.tagName] && !_dtd2.default.$cdata[child.tagName])) {
    ...
    _initEvents: function _initEvents() {
        ...
        (0, _dom2.default)(cont).on('click contextmenu mousedown keydown keyup keypress mouseup mouseover mouseout selectstart', _proxyDomEvent).on('focus blur', _proxyDomEvent).on('mouseup keydown', function (evt) {
    ...
    _selectionChange: function _selectionChange(delay, evt) {
        ...
        var mouseX, mouseY;
        if (_browser2.default.ie && _browser2.default.version < 9 && evt && evt.type == 'mouseup') {
    ...
    _callCmdFn: function _callCmdFn(fnName, args) {
        ...
        var cmdName = args.shift().toLowerCase(),
            cmd,
            cmdFn;
        cmd = this.commands[cmdName] || _editor2.default.commands[cmdName];
    ...
    execCommand: function execCommand(cmdName) {
        ...
        var me = this,
            result,
            cmd = me.commands[cmdName] || _editor2.default.commands[cmdName];
    ...
    hasContents: function hasContents(tags) {
        ...
        if (!_domUtils2.default.isEmptyBlock(this.body)) {
            return true;
        }
        //随时添加,定义的特殊标签如果存在，不能认为是空
        tags = ['div'];
        for (i = 0; ci = tags[i++];) {
            var nodes = _domUtils2.default.getElementsByTagName(this.body, ci);
            for (var n = 0, cn; cn = nodes[n++];) {
                if (_domUtils2.default.isCustomeNode(cn)) {
                    return true;
                }
            }
        }
    ...
    setDisabled: function setDisabled(except, keepDomEvent) {
        ...
        me._except = except ? _utils2.default.isArray(except) ? except : [except] : [];
        ...
        if (!me.bkqueryCommandState) {
            ...
            me.queryCommandState = function (type) {
                if (_utils2.default.indexOf(me._except, type) != -1) {
    ...
    getLang: function getLang(path) {
        var lang = _editor2.default.I18N[this.options.lang];
        if (!lang) {
...
_utils2.default.inherits(Editor, _EventBase2.default);
