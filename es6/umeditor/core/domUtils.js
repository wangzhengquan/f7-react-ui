...
function _defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; } /* eslint quotes : "off"*/
/**
 * @file
...
var attrFix = _browser2.default.ie && _browser2.default.version < 9 ? {
    tabindex: "tabIndex",
    readonly: "readOnly",
    "for": "htmlFor",
    "class": "className",
    ...
    styleBlock = _utils2.default.listToMap(['-webkit-box', '-moz-box', 'block', 'list-item', 'table', 'table-row-group', 'table-header-group', 'table-footer-group', 'table-row', 'table-column-group', 'table-column', 'table-cell', 'table-caption']);
var domUtils = (_domUtils = {
    //节点常量
    NODE_ELEMENT: 1,
    NODE_DOCUMENT: 9,
    NODE_TEXT: 3,
    ...
    //ie6使用其他的会有一段空白出现
    fillChar: _browser2.default.ie && _browser2.default.version == '6' ? '\uFEFF' : '\u200B',
    //-------------------------Node部分--------------------------------
    keys: {
    ...
    findParentByTagName: function findParentByTagName(node, tagNames, includeSelf, excludeFn) {
        tagNames = _utils2.default.listToMap(_utils2.default.isArray(tagNames) ? tagNames : [tagNames]);
    ...
    split: function split(node, offset) {
        ...
        if (_browser2.default.ie && offset == node.nodeValue.length) {
            var next = doc.createTextNode('');
            return domUtils.insertAfter(node, next);
        }
        var retval = node.splitText(offset);
        //ie8下splitText不会跟新childNodes,我们手动触发他的更新
        if (_browser2.default.ie8) {
    ...
    isEmptyInlineElement: function isEmptyInlineElement(node) {
        if (node.nodeType != 1 || !_dtd2.default.$removeEmpty[node.tagName]) {
            return 0;
        }
    ...
    isBlockElm: function isBlockElm(node) {
        return node.nodeType == 1 && (_dtd2.default.$block[node.tagName] || styleBlock[domUtils.getComputedStyle(node, 'display')]) && !_dtd2.default.$nonChild[node.tagName];
    },
    ...
    getElementsByTagName: function getElementsByTagName(node, name, filter) {
        if (filter && _utils2.default.isString(filter)) {
            var className = filter;
            filter = function filter(node) {
                var result = false;
                _utils2.default.trim(className).replace(/[ ]{2,}/g, ' ').split(' ').forEach(function (v) {
                    if ((0, _dom2.default)(node).hasClass(v)) {
                        result = true;
                        return false;
                    }
                });
        ...
        name = _utils2.default.trim(name).replace(/[ ]{2,}/g, ' ').split(' ');
        var arr = [];
     ...
     */
    unSelectable: _browser2.default.ie && _browser2.default.ie9below || _browser2.default.opera ? function (node) {
        //for ie9
    ...
    removeAttributes: function removeAttributes(node, attrNames) {
        attrNames = _utils2.default.isArray(attrNames) ? attrNames : _utils2.default.trim(attrNames).replace(/[ ]{2,}/g, ' ').split(' ');
        for (var i = 0, ci; ci = attrNames[i++];) {
            ...
            switch (ci) {
                ...
                case 'style':
                    node.style.cssText = '';
                    !_browser2.default.ie && node.removeAttributeNode(node.getAttributeNode('style'));
            }
    ...
    getComputedStyle: function getComputedStyle(element, styleName) {
        return _utils2.default.transUnitToPx(_utils2.default.fixColor(styleName, (0, _dom2.default)(element).css(styleName)));
    },
    ...
    removeStyle: function removeStyle(element, name) {
        if (_browser2.default.ie) {
            ...
            if (element.style.removeProperty) {
                ...
                element.style.removeAttribute(_utils2.default.cssStyleToDomStyle(name));
            }
        }
    ...
    getStyle: function getStyle(element, name) {
        var value = element.style[_utils2.default.cssStyleToDomStyle(name)];
        return _utils2.default.fixColor(name, value);
    },
    /**
     * 为元素element设置样式属性值
     * @name setStyle
    ...
    setStyle: function setStyle(element, name, value) {
        element.style[_utils2.default.cssStyleToDomStyle(name)] = value;
        if (!_utils2.default.trim(element.style.cssText)) {
    ...
    isEmptyBlock: function isEmptyBlock(node, reg) {
        ...
        reg = reg || new RegExp('[ \t\r\n' + domUtils.fillChar + ']', 'g');
        if (node[_browser2.default.ie ? 'innerText' : 'textContent'].replace(reg, '').length > 0) {
            return 0;
        }
        for (var n in _dtd2.default.$isNotEmpty) {
    ...
    fillNode: function fillNode(doc, node) {
        var tmpNode = _browser2.default.ie ? doc.createTextNode(domUtils.fillChar) : doc.createElement('br');
        node.innerHTML = '';
...
}, _defineProperty(_domUtils, 'isFillChar', function isFillChar(node, isInStart) {
    return node.nodeType == 3 && !node.nodeValue.replace(new RegExp((isInStart ? '^' : '') + domUtils.fillChar), '').length;
}), _defineProperty(_domUtils, 'isBody', function isBody(node) {
    return (0, _dom2.default)(node).hasClass('edui-body-container');
}), _domUtils);
domUtils.fillCharReg = new RegExp(domUtils.fillChar, 'g');
