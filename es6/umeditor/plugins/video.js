...
/* eslint quotes : "off"*/
_plugins2.default.plugins['video'] = function () {
    var me = this;
    ...
    function creatInsertStr(url, width, height, id, align, toEmbed) {
        return !toEmbed ? '<img ' + (id ? 'id="' + id + '"' : '') + ' width="' + width + '" height="' + height + '" _url="' + url + '" class="edui-faked-video"' + ' src="' + me.options.PluginsEDITOR_HOME_URL + 'themes/default/images/spacer.gif" style="background:url(' + me.options.PluginsEDITOR_HOME_URL + 'themes/default/images/videologo.gif) no-repeat center center; border:1px solid gray;' + (align ? 'float:' + align + ';' : '') + '" />' : '<embed type="application/x-shockwave-flash" class="edui-faked-video" pluginspage="http://www.macromedia.com/go/getflashplayer"' + ' src="' + url + '" width="' + width + '" height="' + height + '"' + (align ? ' style="float:' + align + '"' : '') + ' wmode="transparent" play="true" loop="false" menu="false" allowscriptaccess="never" allowfullscreen="true" >';
    }

    function switchImgAndEmbed(root, img2embed) {
        _utils2.default.each(root.getNodesByTagName(img2embed ? 'img' : 'embed'), function (node) {
            if (node.getAttr('class') == 'edui-faked-video') {
                ...
                node.parentNode.replaceChild(_plugins2.default.uNode.createElement(html), node);
            }
        });
    }
    ...
    me.commands["insertvideo"] = {
        execCommand: function execCommand(cmd, videoObjs) {
            videoObjs = _utils2.default.isArray(videoObjs) ? videoObjs : [videoObjs];
            var html = [],
                id = 'tmpVedio';
