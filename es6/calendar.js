...
require('./resources/less/calendar.less');
var Calendar = function Calendar(params) {
    ...
    var defaults = {
        ...
        header: _params2.default.material,
        footer: _params2.default.material,
    ...
    // RTL inverter
    // var inverter = p.isH ? (PARAMS.rtl ? -1 : 1) : 1;
    var inverter = 1;
        ...
        if (!p.inline && p.params.input) {
            if (p.params.onlyInPopover) toPopover = true;else {
                if (_device2.default.ios) {
                    toPopover = _device2.default.ipad ? true : false;
                } else {
    ...
    p.updateValue = function (onlyHeader) {
        ...
        if (p.input && p.input.length > 0 || _params2.default.material && p.params.header) {
            ...
            if (_params2.default.material && p.params.header && p.container && p.container.length > 0) {
                p.container.find('.picker-calendar-selected-date').text(inputValue);
            }
            if (p.input && p.input.length > 0 && !onlyHeader) {
                (0, _dom2.default)(p.input).val(inputValue);
                (0, _dom2.default)(p.input).trigger('change');
            }
        }
    };
    ...
    p.initCalendarEvents = function () {
        ...
        function handleTouchEnd(e) {
            ...
            if (touchEndTime - touchStartTime < 300) {
                if (Math.abs(touchesDiff) < 10) {
                    ...
                    if (_params2.default.rtl) p.nextMonth();else p.prevMonth();
                } else {
                    if (_params2.default.rtl) p.prevMonth();else p.nextMonth();
                }
            } else {
                if (percentage <= -0.5) {
                    if (_params2.default.rtl) p.prevMonth();else p.nextMonth();
        ...
        function handleDayClick(e) {
            ...
            var day = (0, _dom2.default)(e.target).parents('.picker-calendar-day');
            if (day.length === 0 && (0, _dom2.default)(e.target).hasClass('picker-calendar-day')) {
                day = (0, _dom2.default)(e.target);
            }
        ...
        if (p.params.touchMove) {
            p.wrapper.on(_supportEvents2.default.touchEvents.start, handleTouchStart);
            p.wrapper.on(_supportEvents2.default.touchEvents.move, handleTouchMove);
            p.wrapper.on(_supportEvents2.default.touchEvents.end, handleTouchEnd);
        }
        ...
        p.container[0].f7DestroyCalendarEvents = function () {
            ...
            if (p.params.touchMove) {
                p.wrapper.off(_supportEvents2.default.touchEvents.start, handleTouchStart);
                p.wrapper.off(_supportEvents2.default.touchEvents.move, handleTouchMove);
                p.wrapper.off(_supportEvents2.default.touchEvents.end, handleTouchEnd);
            }
        };
    };
    ...
    p.dateInRange = function (dayDate, range) {
        ...
        if (_dom2.default.isArray(range)) {
    ...
    p.nextMonth = function (transition) {
        ...
        if (nextMonth === p.currentMonth) {
            ...
            var nextMonthHTML = (0, _dom2.default)(p.monthHTML(nextDateTime, 'next')).transform('translate3d(' + (p.isH ? nextMonthTranslate : 0) + '%, ' + (p.isH ? 0 : nextMonthTranslate) + '%, 0)').addClass('picker-calendar-month-next');
    ...
    p.prevMonth = function (transition) {
        ...
        if (prevMonth === p.currentMonth) {
            ...
            var prevMonthHTML = (0, _dom2.default)(p.monthHTML(prevDateTime, 'prev')).transform('translate3d(' + (p.isH ? prevMonthTranslate : 0) + '%, ' + (p.isH ? 0 : prevMonthTranslate) + '%, 0)').addClass('picker-calendar-month-prev');
    ...
    function openOnInput(e) {
        ...
        if (p.opened) return;
        p.open();
        if (p.params.scrollToInput && !isPopover() && !_params2.default.material) {
    ...
    function closeOnHTMLClick(e) {
        ...
        if (p.input && p.input.length > 0) {
            if (e.target !== p.input[0] && (0, _dom2.default)(e.target).parents('.picker-modal').length === 0) p.close();
        } else {
            if ((0, _dom2.default)(e.target).parents('.picker-modal').length === 0) p.close();
        }
    }

    if (p.params.input) {
        p.input = (0, _dom2.default)(p.params.input);
        if (p.input.length > 0) {
    ...
    if (!p.inline && p.params.closeByOutsideClick) (0, _dom2.default)('html').on('click', closeOnHTMLClick);
    ...
    function onPickerClose() {
        ...
        if (p.input && p.input.length > 0) {
            ...
            if (_params2.default.material) p.input.trigger('blur');
        }
    ...
    p.open = function () {
        ...
        if (!p.opened) {
            ...
            if (toPopover) {
                ...
                p.popover = _modals2.default.popover(p.pickerHTML, p.params.input, true);
                p.container = (0, _dom2.default)(p.popover).find('.picker-modal');
                (0, _dom2.default)(p.popover).on('close', function () {
                    onPickerClose();
                });
            } else if (p.inline) {
                p.container = (0, _dom2.default)(p.pickerHTML);
                p.container.addClass('picker-modal-inline');
                (0, _dom2.default)(p.params.container).append(p.container);
            } else {
                p.container = (0, _dom2.default)(_modals2.default.pickerModal(p.pickerHTML));
                (0, _dom2.default)(p.container).on('close', function () {
                    onPickerClose();
                });
            }
            ...
            // Update input value
            if (updateValue) p.updateValue();else if (_params2.default.material && p.value) p.updateValue(true);

            // Material Focus
            if (p.input && p.input.length > 0 && _params2.default.material) {
    ...
    p.close = function () {
        ...
        if (inPopover()) {
            _modals2.default.closeModal(p.popover);
            return;
        } else {
            _modals2.default.closeModal(p.container);
            return;
        }
    };
    ...
    p.destroy = function () {
        ...
        (0, _dom2.default)('html').off('click', closeOnHTMLClick);
    };
...
Calendar.calendar = function (params) {
