...
// Size Navbars
var sizeNavbar = _navbars2.default.sizeNavbar; /*======================================================
                                               ************   Navigation / Router   ************
                                               ======================================================*/
...
var router = {
    ...
    findElement: function findElement(selector, container, view, notCached) {
        container = (0, _dom2.default)(container);
        ...
        if (found.length > 1) {
            ...
            if (found.length > 1) {
                // Search in main view
                found = container.find('.' + _params2.default.viewMainClass + ' ' + selector);
            }
        }
    ...
    preroute: function preroute(view, options) {
        if (_params2.default.preroute && _params2.default.preroute(view, options) === false || view && view.params.preroute && view.params.preroute(view, options) === false) {
            return true;
        } else {
            return false;
        }
    ...
    template7Render: function template7Render(view, options) {
        ...
        if (typeof content === 'string') {
            if (url) {
                if (_template2.default.template7Cache[url] && !options.ignoreCache) t7_template = _template2.default.cache[url];else {
                    t7_template = _template2.default.compile(content);
                    _template2.default.cache[url] = t7_template;
                }
            } else t7_template = _template2.default.compile(content);
        } else if (template) {
        ...
        if (context) t7_ctx = context;else {
            if (contextName) {
                if (contextName.indexOf('.') >= 0) {
                    ...
                    var _ctx = _template2.default.data[_ctx_path[0]];
                    for (var i = 1; i < _ctx_path.length; i++) {
                        if (_ctx_path[i]) _ctx = _ctx[_ctx_path[i]];
                    }
                    t7_ctx = _ctx;
                } else t7_ctx = _template2.default.data[contextName];
            }
            if (!t7_ctx && url) {
                t7_ctx = _template2.default.data['url:' + url];
            }
            if (!t7_ctx && typeof content === 'string' && !template) {
                ...
                if (pageNameMatch) {
                    ...
                    if (page) t7_ctx = _template2.default.data['page:' + page];
                }
            }
            if (!t7_ctx && template && _template2.default.templates) {
                // Try to find matched template name in t7.templates
                for (var templateName in _template2.default.templates) {
                    if (_template2.default.templates[templateName] === template) t7_ctx = _template2.default.data[templateName];
                }
            }
            if (!t7_ctx) t7_ctx = {};
        }
        ...
        if (t7_template && t7_ctx) {
            ...
            if (url) {
                ...
                var query = _dom2.default.parseUrlQuery(url);
                t7_ctx.url_query = {};
...
router._load = function (view, options) {
    ...
    t7_rendered = { content: options.content },
        ...
        // Template 7 compiled template
    pageName = options.pageName,
        viewContainer = (0, _dom2.default)(view.container),
        pagesContainer = (0, _dom2.default)(view.pagesContainer),
    ...
    if (_params2.default.template7Pages && typeof content === 'string' || template) {
        ...
        if (typeof content === 'string' || url && typeof content === 'string') {
            ...
            if ('length' in content && content.length > 1) {
                for (var ci = 0; ci < content.length; ci++) {
                    (0, _dom2.default)(router.temporaryDom).append(content[ci]);
                }
            } else {
                (0, _dom2.default)(router.temporaryDom).append(content);
            }
        }
    }
    ...
    if (options.reload) {
        ...
        if (pagesInView.length > 1) {
            for (i = 0; i < pagesInView.length - 1; i++) {
                (0, _dom2.default)(pagesInView[i]).addClass('cached');
            }
        ...
        if (options.reload) {
            ...
            if (oldNavbarInner.length > 0) {
                for (i = 0; i < oldNavbarInner.length - 1; i++) {
                    (0, _dom2.default)(oldNavbarInner[i]).addClass('cached');
                }
    ...
    if (!url) {
        ...
        if (isDynamicPage) url = '#' + _params2.default.dynamicPageUrl.replace(/{{name}}/g, newPageName).replace(/{{index}}/g, view.history.length - (options.reload ? 1 : 0));else url = '#' + newPageName;
        // if (!view.params.domCache) {
    ...
    // Page Init Events
    _pages2.default.pageInitCallback(view, {
    ...
    // Navbar init event
    if (dynamicNavbar) {
        _navbars2.default.navbarInitCallback(view, newPage[0], navbar[0], newNavbarInner[0], url, options.reload ? reloadPosition : 'right');
    }

    var clickBack = function clickBack(event) {
        ...
        if (clicked.hasClass('back') || template) {
            ...
            if (_params2.default.template7Pages) {
            ...
            if (template && template in _template2.default.templates) {
                options.template = _template2.default.templates[template];
            }
    ...
    _pages2.default.pageAnimCallback('before', view, {
        pageContainer: newPage[0],
        url: url,
        position: 'right',
        oldPage: oldPage,
    ...
    function afterAnimation() {
        ...
        _pages2.default.pageAnimCallback('after', view, {
            pageContainer: newPage[0],
            url: url,
            position: 'right',
            oldPage: oldPage,
    ...
    if (animatePages) {
        ...
        if (_params2.default.material && _params2.default.materialPageLoadDelay) {
            setTimeout(function () {
                router.animatePages(oldPage, newPage, 'to-left', view);
            }, _params2.default.materialPageLoadDelay);
        } else {
            router.animatePages(oldPage, newPage, 'to-left', view);
        }
...
router.load = function (view, options) {
    ...
    if (pageName) {
        if (pageName.indexOf('?') > 0) {
            options.query = _dom2.default.parseUrlQuery(pageName);
...
router._back = function (view, options) {
    ...
    var viewContainer = (0, _dom2.default)(view.container),
        pagesContainer = (0, _dom2.default)(view.pagesContainer),
        pagesInView = pagesContainer.children('.page:not(.cached)'),
        oldPage,
        newPage,
    ...
    // Animation
    function afterAnimation() {
        _pages2.default.pageBackCallback('after', view, {
    ...
    function animateBack() {
        ...
        _pages2.default.pageBackCallback('before', view, {
            pageContainer: oldPage[0],
            url: url,
            position: 'center',
            oldPage: oldPage,
    ...
    function setPages() {
        ...
        if (force) {
            ...
            pageToRemove = (0, _dom2.default)(pagesInView[pagesInView.length - 2]);

            if (dynamicNavbar) navbarToRemove = (0, _dom2.default)(pageToRemove[0] && pageToRemove[0].f7RelatedNavbar || navbarInners[navbarInners.length - 2]);
            if (view.initialPages.indexOf(pageToRemove[0]) >= 0) {
                ...
                if (pageToRemove.length) {
                    _pages2.default.pageRemoveCallback(view, pageToRemove[0], 'right');
                    if (removeNavbar) {
                        _navbars2.default.navbarRemoveCallback(view, pageToRemove[0], navbar[0], navbarToRemove[0]);
                    }
                    pageToRemove.remove();
        ...
        oldPage = (0, _dom2.default)(pagesInView[pagesInView.length - 1]);
        if (oldPage[0] === newPage[0]) {
            oldPage = pagesContainer.children('.page.page-on-center');
            if (oldPage.length === 0 && view.activePage) oldPage = (0, _dom2.default)(view.activePage.container);
        }

        if (dynamicNavbar && !oldNavbarInner) {
            oldNavbarInner = (0, _dom2.default)(navbarInners[navbarInners.length - 1]);
        ...
        if (dynamicNavbar) {
            _navbars2.default.navbarInitCallback(view, newPage[0], navbar[0], newNavbarInner[0], url, 'right');
        }
    ...
    if (pagesInView.length > 1 && !force) {
        ...
        newPage = (0, _dom2.default)(pagesInView[pagesInView.length - 2]);
        oldPage = (0, _dom2.default)(pagesInView[pagesInView.length - 1]);
        ...
        if (view.params.dynamicNavbar) {
            ...
            newNavbarInner = (0, _dom2.default)(navbarInners[0]);
            oldNavbarInner = (0, _dom2.default)(navbarInners[1]);
    ...
    if (!force) {
        ...
        if (content) {
            ...
            newPage = (0, _dom2.default)(viewContainer).find('.page[data-page="' + pageName + '"]');
            if (view.params.dynamicNavbar) {
                newNavbarInner = (0, _dom2.default)(viewContainer).find('.navbar-inner[data-page="' + pageName + '"]');
                if (newNavbarInner.length === 0 && newPage[0].f7RelatedNavbar) {
                    newNavbarInner = (0, _dom2.default)(newPage[0].f7RelatedNavbar);
                }
                if (newNavbarInner.length === 0 && newPage[0].f7PageData) {
                    newNavbarInner = (0, _dom2.default)(newPage[0].f7PageData.navbarInnerContainer);
                }
            }
            setPages();
            return;
...
router.back = function (view, options) {
    ...
    var pagesInView = (0, _dom2.default)(view.pagesContainer).find('.page:not(.cached)');
...
router.afterBack = function (view, oldPage, newPage) {
    ...
    oldPage = (0, _dom2.default)(oldPage);
    newPage = (0, _dom2.default)(newPage);

    if (view.initialPages.indexOf(oldPage[0]) >= 0) {
        oldPage.removeClass('page-from-center-to-right').addClass('cached');
    } else {
        _pages2.default.pageRemoveCallback(view, oldPage[0], 'right');
    ...
    if (view.params.dynamicNavbar) {
        var inners = (0, _dom2.default)(view.container).find('.navbar-inner:not(.cached)');
        var oldNavbar = (0, _dom2.default)(oldPage[0].f7RelatedNavbar || inners[1]);
        if (view.initialNavbars.indexOf(oldNavbar[0]) >= 0) {
            oldNavbar.removeClass('navbar-from-center-to-right').addClass('cached');
        } else {
            _navbars2.default.navbarRemoveCallback(view, oldPage[0], undefined, oldNavbar[0]);
            oldNavbar.remove();
        }
        newNavbar = (0, _dom2.default)(inners[0]).removeClass('navbar-on-left navbar-from-left-to-center').addClass('navbar-on-center');
    }

    // Remove pages in dom cache
    (0, _dom2.default)(view.container).find('.page.cached').each(function () {
        // debugger
        var page = (0, _dom2.default)(this);
        var index = page.index();
        var pageUrl = page[0].f7PageData && page[0].f7PageData.url;
        if (pageUrl && view.history.indexOf(pageUrl) < 0 && view.initialPages.indexOf(this) < 0) {
            _pages2.default.pageRemoveCallback(view, page[0], 'right');
            if (page[0].f7RelatedNavbar && view.params.dynamicNavbar) {
                _navbars2.default.navbarRemoveCallback(view, page[0], undefined, page[0].f7RelatedNavbar);
            }
            page.remove();
            if (page[0].f7RelatedNavbar && view.params.dynamicNavbar) (0, _dom2.default)(page[0].f7RelatedNavbar).remove();
        }
    });
    ...
    if (view.params.preloadPreviousPage) {
        if (view.history.length > 1) {
            ...
            if (preloadUrl && view.pagesCache[preloadUrl]) {
                // Load by page name
                previousPage = (0, _dom2.default)(view.container).find('.page[data-page="' + view.pagesCache[preloadUrl] + '"]');
                if (previousPage.next('.page')[0] !== newPage[0]) previousPage.insertBefore(newPage);
                if (newNavbar) {
                    previousNavbar = (0, _dom2.default)(view.container).find('.navbar-inner[data-page="' + view.pagesCache[preloadUrl] + '"]');
