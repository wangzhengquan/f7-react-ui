...
function _getAttr(el, key) {
	...
	if (!_GET_SET_ATTRIBUTE && el.nodeName.toLowerCase() != 'script') {
		...
		var list = HtmlHelper.getAttrList(HtmlHelper.unescape(div.innerHTML));
		if (key in list) {
			val = list[key];
		}
	} else {
		// IE8设置<meta http-equiv="X-UA-Compatible" content="IE=EmulateIE7" />，抛出异常
		try {
...
function _queryAll(expr, root) {
	...
	if (exprList.length > 1) {
		var mergedResults = [];
		K.each(exprList, function() {
			K.each(_queryAll(this, root), function() {
				if (K.inArray(this, mergedResults) < 0) {
