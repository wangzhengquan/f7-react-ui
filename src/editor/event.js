...
Util.extend(KEvent, {
	init : function(el, event) {
		...
		self.event = event;
		Util.each(_EVENT_PROPS, function(key, val) {
			self[val] = event[val];
		});
		if (!self.target) {
...
var _eventExpendo = 'kindeditor_' + Vars.TIME, _eventId = 0, _eventData = {};
...
function _bind(el, type, fn) {
	...
	if (events.length === 0) {
		events[0] = function(e) {
			...
			Util.each(events, function(i, event) {
				if (i > 0 && event) {
...
function _unbind(el, type, fn) {
	...
	if (type === undefined) {
		if (id in _eventData) {
			Util.each(_eventData[id], function(key, events) {
	...
	if (events && events.length > 0) {
		...
		var count = 0;
		Util.each(_eventData[id], function() {
			count++;
		});
		if (count < 2) {
			delete _eventData[id];
...
var _destory = function() {
	Util.each(_eventData, function(key, events) {
		if (events.el) {
			_unbind(events.el);
		}
	});
...
KEvent.ctrl = _ctrl;
KEvent.bind = _bind;
KEvent.unbind = _unbind;
KEvent.fire = _fire;
KEvent.destory = _destory;
KEvent.INPUT_KEY_MAP = _INPUT_KEY_MAP;
