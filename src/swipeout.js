...
SwipeOut.initSwipeout = function (swipeoutEl) {
    ...
    $(document).on(SupportEvents.touchEvents.start, function (e) {
        if (SwipeOut.swipeoutOpenedEl) {
            var target = $(e.target);
            if (!(
                SwipeOut.swipeoutOpenedEl.is(target[0]) ||
                target.parents('.swipeout').is(SwipeOut.swipeoutOpenedEl) ||
    ...
    if (swipeoutEl) {
        $(swipeoutEl).on(SupportEvents.touchEvents.start, handleTouchStart);
        $(swipeoutEl).on(SupportEvents.touchEvents.move, handleTouchMove);
        $(swipeoutEl).on(SupportEvents.touchEvents.end, handleTouchEnd);
    }
    else {
        $(document).on(SupportEvents.touchEvents.start, '.list-block li.swipeout', handleTouchStart);
        $(document).on(SupportEvents.touchEvents.move, '.list-block li.swipeout', handleTouchMove);
        $(document).on(SupportEvents.touchEvents.end, '.list-block li.swipeout', handleTouchEnd);
    }
