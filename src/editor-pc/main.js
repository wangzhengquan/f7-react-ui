...
function _getImageFromRange(range, fn) {
	...
	if (!VARS.WEBKIT && !range.isControl()) {
		return;
	}
	var img = KNode(sc.childNodes[so]);
	if (!img || img.name != 'img') {
		return;
	}
	if (fn(img)) {
...
function _bindContextmenuEvent() {
	...
	KNode(doc).contextmenu(function(e) {
		if (self.menu) {
			self.hideMenu();
		}
		...
		Util.each(self._contextmenus, function() {
			if (this.title == '-') {
				items.push(this);
				return;
			}
		...
		if (items.length > 0) {
			e.preventDefault();
			var pos = KNode(self.edit.iframe).pos(),
				menu = new KMenu({
					x : pos.x + e.clientX,
					y : pos.y + e.clientY,
					width : maxWidth,
			...
			Util.each(items, function() {
				if (this.title) {
					menu.addItem(this);
				}
			});
			// 下拉菜单超过可视区域时调整菜单位置
			var docEl = KNode.docElement(menu.doc),
				menuHeight = menu.div.height();
			if (e.clientY + menuHeight >= docEl.clientHeight - 100) {
				menu.pos(menu.x, Util.removeUnit(menu.y) - menuHeight);
			}
...
function _bindNewlineEvent() {
	...
	if (VARS.IE && newlineTag !== 'br') {
		return;
	}
	if (VARS.GECKO && VARS.V < 3 && newlineTag !== 'p') {
		return;
	}
	if (VARS.OPERA && VARS.V < 9) {
		return;
	}
	var brSkipTagMap = Util.toMap('h1,h2,h3,h4,h5,h6,pre,li'),
		pSkipTagMap = Util.toMap('p,h1,h2,h3,h4,h5,h6,pre,li,blockquote');
	// 取得range的block标签名
	function getAncestorTagName(range) {
		var ancestor = KNode(range.commonAncestor());
		while (ancestor) {
	...
	KNode(doc).keydown(function(e) {
		...
		if (newlineTag === 'br' && !brSkipTagMap[tagName]) {
			e.preventDefault();
			self.insertHtml('<br />' + (VARS.IE && VARS.V < 9 ? '' : '\u200B'));
			return;
		}
		// p
		if (!pSkipTagMap[tagName]) {
			KCmd._nativeCommand(doc, 'formatblock', '<p>');
		}
	});
	...
	KNode(doc).keydown(function(e) {
		if (e.which == 9) {
			...
			if (range.collapsed && range.startContainer.nodeType == 1) {
				range.insertNode(KNode('<div></div>', doc)[0]);
				cmd.select();
			}
			self.insertHtml('&nbsp;&nbsp;&nbsp;&nbsp;');
		}
...
function _bindFocusEvent() {
	var self = this;
	KNode(self.edit.textarea[0], self.edit.win).focus(function(e) {
		if (self.afterFocus) {
...
function _removeBookmarkTag(html) {
	return Util.trim(html.replace(/<span [^>]*id="?__kindeditor_bookmark_\w+_\d+__"?[^>]*><\/span>/ig, ''));
}
...
function _undoToRedo(fromStack, toStack) {
	...
	if (edit.designMode) {
		...
		KNode(body).html(_removeBookmarkTag(prev.html));
	}
	return self;
}
...
function KEditor(options) {
	...
	if (VARS.MOBILE && (!VARS.IOS || VARS.V < 534)) {
...
KEditor.prototype = {
	...
	loadPlugin : function(name, fn) {
		...
		if (_plugins[name]) {
			// JS加载中，等待JS加载完成
			if (!Util.isFunction(_plugins[name])) {
				setTimeout(function() {
		...
		// _loadScript(self.pluginsPath + name + '/' + name + '.js?ver=' + encodeURIComponent(K.DEBUG ? _TIME : VARS.VERSION), function() {
	...
	handler : function(key, fn) {
		...
		var i =0;
		Util.each(self._handlers[key], function() {
	...
	create : function() {
		...
		if ((VARS.IE && VARS.V < 8) || VARS.QUIRKS) {
			height = Util.addUnit(Util.removeUnit(height) + 2);
		}
	...
	exec : function(key) {
		...
		var self = this, cmd = self.cmd,
			changeFlag = Util.inArray(key, 'selectall,copy,paste,print'.split(',')) < 0;
		if (changeFlag) {
			self.addBookmark(false);
		}
		cmd[key].apply(cmd, Util.toArray(arguments, 1));
	...
	addBookmark : function(checkSize) {
		checkSize = Util.undef(checkSize, true);
		var self = this, edit = self.edit,
			body = edit.doc.body,
...
_plugin('core', function() {
	...
	self.afterCreate(function() {
		...
		var el = KNode(self.srcElement), hasForm = false;
		while ((el = el.parent())) {
			if (el[0].nodeName && el[0].nodeName.toLowerCase() === 'form') {
				hasForm = true;
				break;
			}
		}
		if (hasForm) {
			el.bind('submit', function() {
				...
				KNode(window).bind('unload', function() {
					self.edit.textarea.remove();
				});
			});
			var resetBtn = KNode('[type="reset"]', el);
	...
	if (self.fullscreenShortcut) {
		var loaded = false;
		self.afterCreate(function() {
			KNode(self.edit.doc, self.edit.textarea).keyup(function(e) {
				if (e.which == 27) {
	...
	// undo, redo
	Util.each('undo,redo'.split(','), function(i, name) {
	...
	self.clickToolbar('formatblock', function() {
		...
		Util.each(blocks, function(key, val) {
	...
	self.clickToolbar('fontsize', function() {
		...
		Util.each(self.fontSizeTable, function(i, val) {
			menu.addItem({
				title : '<span style="font-size:' + val + ';" unselectable="on">' + val + '</span>',
				height : Util.removeUnit(val) + 12,
				checked : curVal === val,
				click : function() {
	...
	Util.each('forecolor,hilitecolor'.split(','), function(i, name) {
		self.clickToolbar(name, function() {
			self.createMenu({
				name : name,
	...
	// cut,copy,paste
	Util.each(('cut,copy,paste').split(','), function(i, name) {
		self.clickToolbar(name, function() {
			self.focus();
			try {
				self.exec(name, null);
	...
	self.clickToolbar('about', function() {
		var html = '<div style="margin:20px;">' +
			'<div>KindEditor ' + VARS.VERSION + '</div>' +
	...
	Util.each('link,image,flash,media,anchor'.split(','), function(i, name) {
		var uName = name.charAt(0).toUpperCase() + name.substr(1);
		Util.each('edit,delete'.split(','), function(j, val) {
			self.addContextmenu({
	...
	Util.each(('prop,cellprop,colinsertleft,colinsertright,rowinsertabove,rowinsertbelow,rowmerge,colmerge,' +
	'rowsplit,colsplit,coldelete,rowdelete,insert,delete').split(','), function(i, val) {
		var cond = Util.inArray(val, ['prop', 'delete']) < 0 ? self.plugin.getSelectedCell : self.plugin.getSelectedTable;
		self.addContextmenu({
	...
	self.addContextmenu({ title : '-' });
	// other
	Util.each(('selectall,justifyleft,justifycenter,justifyright,justifyfull,insertorderedlist,' +
	...
	self.afterCreate(function() {
		...
		function movePastedData() {
			...
			cmd.select();
			if (VARS.WEBKIT) {
				KNode('div.' + cls, div).each(function() {
					KNode(this).after('<br />').remove(true);
				});
				KNode('span.Apple-style-span', div).remove(true);
				KNode('span.Apple-tab-span', div).remove(true);
				// Bugfix: https://github.com/kindsoft/kindeditor/issues/8
				KNode('span[style]', div).each(function() {
					if (KNode(this).css('white-space') == 'nowrap') {
						KNode(this).remove(true);
					}
				});
				KNode('meta', div).remove();
			}
			...
			if (self.pasteType === 2) {
				...
				if (/schemas-microsoft-com|worddocument|mso-\w+/i.test(html)) {
					html = HtmlHelper.clearMsWord(html, self.filterMode ? self.htmlTags : CONFIG.htmlTags);
				} else {
					html = HtmlHelper.formatHtml(html, self.filterMode ? self.htmlTags : null);
		...
		KNode(doc.body).bind('paste', function(e){
			if (self.pasteType === 0) {
				e.stop();
				return;
			}
			...
			pasting = true;
			KNode('div.' + cls, doc).remove();
			cmd = self.cmd.selection();
			bookmark = cmd.range.createBookmark();
			div = KNode('<div class="' + cls + '"></div>', doc).css({
				position : 'absolute',
				width : '1px',
				height : '1px',
				overflow : 'hidden',
				left : '-1981px',
				top : KNode(bookmark.start).pos().y + 'px',
				'white-space' : 'nowrap'
			});
			KNode(doc.body).append(div);
			if (VARS.IE) {
	...
	self.beforeGetHtml(function(html) {
		...
		return html.replace(/(<(?:noscript|noscript\s[^>]*)>)([\s\S]*?)(<\/noscript>)/ig, function($0, $1, $2, $3) {
			return $1 + HtmlHelper.unescape($2).replace(/\s+/g, ' ') + $3;
		})
		.replace(/<img[^>]*class="?ke-(flash|rm|media)"?[^>]*>/ig, function(full) {
			var imgAttrs = HtmlHelper.getAttrList(full);
			var styles = HtmlHelper.getCssList(imgAttrs.style || '');
			var attrs = HtmlHelper.mediaAttrs(imgAttrs['data-ke-tag']);
			var width = Util.undef(styles.width, '');
			var height = Util.undef(styles.height, '');
			if (/px/i.test(width)) {
				width = Util.removeUnit(width);
			}
			if (/px/i.test(height)) {
			...
			attrs.width = Util.undef(imgAttrs.width, width);
			attrs.height = Util.undef(imgAttrs.height, height);
			return HtmlHelper.mediaEmbed(attrs);
		})
		...
		.replace(/(<[^>]*)data-ke-src="([^"]*)"([^>]*>)/ig, function(full, start, src) {
			full = full.replace(/(\s+(?:href|src)=")[^"]*(")/i, function($0, $1, $2) {
				return $1 + HtmlHelper.unescape(src) + $2;
			});
	...
	self.beforeSetHtml(function(html) {
		...
		if (VARS.IE && VARS.V <= 8) {
			html = html.replace(/<input[^>]*>|<(select|button)[^>]*>[\s\S]*?<\/\1>/ig, function(full) {
				var attrs = HtmlHelper.getAttrList(full);
				var styles = HtmlHelper.getCssList(attrs.style || '');
		...
		return html.replace(/<embed[^>]*type="([^"]+)"[^>]*>(?:<\/embed>)?/ig, function(full) {
			var attrs = HtmlHelper.getAttrList(full);
			attrs.src = Util.undef(attrs.src, '');
			attrs.width = Util.undef(attrs.width, 0);
			attrs.height = Util.undef(attrs.height, 0);
			return HtmlHelper.mediaImg(self.themesPath + 'common/blank.gif', attrs);
		})
		...
		.replace(/(<[^>]*)(href|src)="([^"]*)"([^>]*>)/ig, function(full, start, key, src, end) {
			...
			full = start + key + '="' + src + '"' + ' data-ke-src="' + HtmlHelper.escape(src) + '"' + end;
			return full;
		})
		...
		.replace(/<table[^>]*\s+border="0"[^>]*>/ig, function(full) {
			...
			return HtmlHelper.addClassToTag(full, 'ke-zeroborder');
		});
	});
});
