...
var Calendar = function (params) {
    ...
    var defaults = {
        ...
        header: PARAMS.material,
        footer: PARAMS.material,
        toolbarTemplate:
            '<div class="toolbar">' +
    ...
    p.updateValue = function (onlyHeader) {
        ...
        if ((p.input && p.input.length > 0) || (PARAMS.material && p.params.header)) {
            ...
            if (PARAMS.material && p.params.header && p.container && p.container.length > 0) {
    ...
    p.initCalendarEvents = function () {
        ...
        function handleTouchEnd (e) {
            ...
            if (touchEndTime - touchStartTime < 300) {
                if (Math.abs(touchesDiff) < 10) {
                    ...
                    if (PARAMS.rtl) p.nextMonth();
                    else p.prevMonth();
                }
                else {
                    if (PARAMS.rtl) p.prevMonth();
                    else p.nextMonth();
                }
            }
            else {
        ...
        if (p.params.touchMove) {
            p.wrapper.on(SupportEvents.touchEvents.start, handleTouchStart);
            p.wrapper.on(SupportEvents.touchEvents.move, handleTouchMove);
            p.wrapper.on(SupportEvents.touchEvents.end, handleTouchEnd);
        }
        ...
        p.container[0].f7DestroyCalendarEvents = function () {
            ...
            if (p.params.touchMove) {
                p.wrapper.off(SupportEvents.touchEvents.start, handleTouchStart);
                p.wrapper.off(SupportEvents.touchEvents.move, handleTouchMove);
                p.wrapper.off(SupportEvents.touchEvents.end, handleTouchEnd);
            }
        };
    ...
    function openOnInput(e) {
        ...
        if (p.opened) return;
        p.open();
        if (p.params.scrollToInput && !isPopover() && !PARAMS.material) {
    ...
    function onPickerClose() {
        ...
        if (p.input && p.input.length > 0) {
            ...
            if (PARAMS.material) p.input.trigger('blur');
        }
    ...
    p.open = function () {
        ...
        if (!p.opened) {
            ...
            if (toPopover) {
                ...
                p.popover = Modals.popover(p.pickerHTML, p.params.input, true);
                ...
                p.container = $(Modals.pickerModal(p.pickerHTML));
                $(p.container)
                .on('close', function () {
                    onPickerClose();
                });
            ...
            else if (PARAMS.material && p.value) p.updateValue(true);

            // Material Focus
            if (p.input && p.input.length > 0 && PARAMS.material) {
    ...
    p.close = function () {
        ...
        if (inPopover()) {
            Modals.closeModal(p.popover);
            return;
        }
        else {
            Modals.closeModal(p.container);
            return;
        }
    };
